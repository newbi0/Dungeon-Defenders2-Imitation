...
HRESULT CWind::Ready_GameObject_Prototype(void)
{
...
HRESULT CWind::Ready_GameObject_Clone(const _vec3 * pPos)
{
	...
	m_pBufferCom->Resize_Buffer(_vec2(-4.f, -6.f), _vec2(6.f, 4.f));
...
HRESULT CWind::Ready_GameObject_Clone(const _matrix & matWorld)
{
	Ready_GameObject_Clone();
	return NOERROR;
}

HRESULT CWind::Ready_Component(void)
{
	...
	// For.Com_Textrue
	m_pTextureCom = (Engine::CTexture*)m_pComponent_Manager->Clone_Component(SCENE_STAGE, L"Com_Texture_Effect_Wind");
...
_int CWind::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (nullptr == m_pRendererCom)
		return _EXIT;
...
void CWind::Render_GameObject(void)
{
	if (nullptr == m_pTransformCom
...
CGameObject * CWind::Clone(const _vec3 * pPos)
{
	CGameObject*	pInstance = new CWind(*this);

	if (FAILED(pInstance->Ready_GameObject_Clone(pPos)))
	{
		_MSG_BOX("CWind Clone Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CWind::Clone(const _matrix & matWorld)
{
	CGameObject*	pInstance = new CWind(*this);
