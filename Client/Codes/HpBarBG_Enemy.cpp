...
CHpBarBG_Enemy::CHpBarBG_Enemy(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CUI(pGraphic_Device)
{
}

CHpBarBG_Enemy::CHpBarBG_Enemy(const CHpBarBG_Enemy & rhs)
	: CUI(rhs)
{
}

HRESULT CHpBarBG_Enemy::Ready_GameObject_Prototype(void)
{
...
HRESULT CHpBarBG_Enemy::Ready_GameObject_Clone(const _vec3 * pPos)
{
	...
	m_pBufferCom->Resize_Buffer(_vec2(-1.f, 0.f), _vec2(1.f, 0.1f));

	return NOERROR;
}

HRESULT CHpBarBG_Enemy::Ready_GameObject_Clone(const _matrix & matWorld)
{
	Ready_GameObject_Clone();
	return NOERROR;
}

HRESULT CHpBarBG_Enemy::Ready_Component(void)
{
...
void CHpBarBG_Enemy::Set_UnitHpBar(CUnit * pUnit)
{
	if (nullptr == pUnit)
		return;
...
_int CHpBarBG_Enemy::Update_GameObject(const _float & fTimeDelta)
{
...
_int CHpBarBG_Enemy::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (nullptr == m_pRendererCom)
		return _EXIT;
...
void CHpBarBG_Enemy::Render_GameObject(void)
{
	if (nullptr == m_pTransformCom
...
CHpBarBG_Enemy * CHpBarBG_Enemy::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CHpBarBG_Enemy* pInstance = new CHpBarBG_Enemy(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		_MSG_BOX("CHpBarBG_Enemy Created Failed");
...
CGameObject * CHpBarBG_Enemy::Clone(const _vec3 * pPos)
{
	CGameObject*	pInstance = new CHpBarBG_Enemy(*this);

	if (FAILED(pInstance->Ready_GameObject_Clone(pPos)))
	{
		_MSG_BOX("CHpBarBG_Enemy Clone Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CHpBarBG_Enemy::Clone(const _matrix & matWorld)
{
	CGameObject*	pInstance = new CHpBarBG_Enemy(*this);
