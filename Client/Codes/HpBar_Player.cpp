...
CHpBar_Player::CHpBar_Player(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CUI(pGraphic_Device)
{
}
...
HRESULT CHpBar_Player::Ready_GameObject_Prototype(const _bool& isLeft)
{
	m_isLeft = isLeft;
...
HRESULT CHpBar_Player::Ready_GameObject_Clone(const _vec3 * pPos)
{
...
HRESULT CHpBar_Player::Ready_GameObject_Clone(const _matrix & matWorld)
{
	Ready_GameObject_Clone();
	return NOERROR;
}

HRESULT CHpBar_Player::Ready_Component(void)
{
	...
	// For.Com_Textrue
	m_pTextureCom = (Engine::CTexture*)m_pComponent_Manager->Clone_Component(SCENE_STAGE, L"Com_Texture_HpBar_Player");
...
void CHpBar_Player::Set_UnitHpBar(CUnit * pUnit)
{
	if (nullptr == pUnit)
		return;
...
_int CHpBar_Player::Update_GameObject(const _float & fTimeDelta)
{
...
_int CHpBar_Player::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (nullptr == m_pRendererCom)
		return _EXIT;
...
void CHpBar_Player::Render_GameObject(void)
{
	if (nullptr == m_pTransformCom
	...
	if (FAILED(pEffect->SetFloat("g_fY", m_fHpY)))
		return;
...
CHpBar_Player * CHpBar_Player::Create(LPDIRECT3DDEVICE9 pGraphic_Device, const _bool& isLeft)
{
	CHpBar_Player* pInstance = new CHpBar_Player(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype(isLeft)))
	{
		_MSG_BOX("CHpBar_Player Created Failed");
...
CGameObject * CHpBar_Player::Clone(const _vec3 * pPos)
{
	CGameObject*	pInstance = new CHpBar_Player(*this);

	if (FAILED(pInstance->Ready_GameObject_Clone(pPos)))
	{
		_MSG_BOX("CHpBar_Player Clone Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CHpBar_Player::Clone(const _matrix & matWorld)
{
	CGameObject*	pInstance = new CHpBar_Player(*this);
