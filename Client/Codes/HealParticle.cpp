...
CHealParticle::CHealParticle(LPDIRECT3DDEVICE9 CHealParticle)
	: CSkill(CHealParticle)
{
}

CHealParticle::CHealParticle(const CHealParticle & rhs)
	: CSkill(rhs)
{
}

HRESULT CHealParticle::Ready_GameObject_Prototype(void)
{
...
HRESULT CHealParticle::Ready_GameObject_Clone(const _vec3 * pPos)
{
	...
	m_pBufferCom->Resize_Buffer(_vec2(-0.2f, -0.2f), _vec2(0.2f, 0.2f));

	if (nullptr != pPos)
	{
		m_pTransformCom->Set_Infomation(CTransform::INFO_POSITION, pPos);
		m_fOriginHight = (*pPos).y;
	}

	return NOERROR;
}

HRESULT CHealParticle::Ready_GameObject_Clone(const _matrix & matWorld)
{
	Ready_GameObject_Clone();
	return NOERROR;
}

HRESULT CHealParticle::Ready_Component(void)
{
	...
	// For.Com_Textrue
	m_pTextureCom = (Engine::CTexture*)m_pComponent_Manager->Clone_Component(SCENE_STAGE, L"Com_Texture_Effect_HealParticle");
...
_int CHealParticle::Update_GameObject(const _float & fTimeDelta)
{
	m_pTransformCom->Go_Direction(_vec3(0.f, 1.f, 0.f), fTimeDelta);
	...
	//일정 높이 이상일 때 없애기
	if (4.f < vPos.y - m_fOriginHight)
		Set_isDead(true);

	return _RUN;
}

_int CHealParticle::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (nullptr == m_pRendererCom)
		return _EXIT;
...
void CHealParticle::Render_GameObject(void)
{
	if (nullptr == m_pTransformCom
...
CHealParticle * CHealParticle::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CHealParticle* pInstance = new CHealParticle(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		_MSG_BOX("CHealParticle Created Failed");
...
CGameObject * CHealParticle::Clone(const _vec3 * pPos)
{
	CGameObject*	pInstance = new CHealParticle(*this);

	if (FAILED(pInstance->Ready_GameObject_Clone(pPos)))
	{
		_MSG_BOX("CHealParticle Clone Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CHealParticle::Clone(const _matrix & matWorld)
{
	CGameObject*	pInstance = new CHealParticle(*this);
