...
CHpBarBG_Boss::CHpBarBG_Boss(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CUI(pGraphic_Device)
{
}

CHpBarBG_Boss::CHpBarBG_Boss(const CHpBarBG_Boss & rhs)
	: CUI(rhs)
{
}

HRESULT CHpBarBG_Boss::Ready_GameObject_Prototype(void)
{
...
HRESULT CHpBarBG_Boss::Ready_GameObject_Clone(const _vec3 * pPos)
{
	...
	m_pTransformCom->Set_Infomation(CTransform::INFO_POSITION, &_vec3(-0.022f, 0.675f, 0.f));

	return NOERROR;
}

HRESULT CHpBarBG_Boss::Ready_GameObject_Clone(const _matrix & matWorld)
{
	Ready_GameObject_Clone();
	return NOERROR;
}

HRESULT CHpBarBG_Boss::Ready_Component(void)
{
...
_int CHpBarBG_Boss::Update_GameObject(const _float & fTimeDelta)
{
	return _RUN;
}

_int CHpBarBG_Boss::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (nullptr == m_pRendererCom)
		return _EXIT;
...
void CHpBarBG_Boss::Render_GameObject(void)
{
	if (nullptr == m_pTransformCom
...
CHpBarBG_Boss * CHpBarBG_Boss::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CHpBarBG_Boss* pInstance = new CHpBarBG_Boss(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		_MSG_BOX("CHpBarBG_Boss Created Failed");
...
CGameObject * CHpBarBG_Boss::Clone(const _vec3 * pPos)
{
	CGameObject*	pInstance = new CHpBarBG_Boss(*this);

	if (FAILED(pInstance->Ready_GameObject_Clone(pPos)))
	{
		_MSG_BOX("CHpBarBG_Boss Clone Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CHpBarBG_Boss::Clone(const _matrix & matWorld)
{
	CGameObject*	pInstance = new CHpBarBG_Boss(*this);
