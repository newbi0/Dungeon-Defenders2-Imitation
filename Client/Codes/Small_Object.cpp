...
CSmall_Object::CSmall_Object(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{
}

CSmall_Object::CSmall_Object(const CSmall_Object & rhs)
	: CGameObject(rhs)
	, m_vHitZonePos(rhs.m_vHitZonePos)
	, m_fHitZoneSize(rhs.m_fHitZoneSize)
{
}

HRESULT CSmall_Object::Ready_GameObject_Prototype(const _tchar* pMashTag)
{
...
HRESULT CSmall_Object::Ready_GameObject_Clone(const _vec3* pPos)
{
...
HRESULT CSmall_Object::Ready_GameObject_Clone(const _matrix& matWorld)
{
	m_pRendererCom = (Engine::CRenderer*)Get_Component(L"Com_Renderer");
	m_pRendererCom->AddRef();
...
HRESULT CSmall_Object::Ready_Component(const _tchar* pMashTag)
{
...
HRESULT CSmall_Object::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
...
HRESULT CSmall_Object::SetUp_LightAndMaterialInfo_OnShader(LPD3DXEFFECT pEffect)
{
	//LightInfo
...
_int CSmall_Object::Update_GameObject(const _float & fTimeDelta)
{
	if (m_eHitZoneType != TYPE_END && true == m_isHitZone)
	{
...
_int CSmall_Object::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (nullptr == m_pRendererCom)
		return -1;
...
void CSmall_Object::Render_GameObject(void)
{
	if (nullptr == m_pTransformCom
	...
	{
		_MSG_BOX("CSmall_Object SetUp_ConstantTable Failed");
		return;
	}
...
CSmall_Object * CSmall_Object::Create(LPDIRECT3DDEVICE9 pGraphic_Device, const _tchar* pMashTag)
{
	CSmall_Object* pInstance = new CSmall_Object(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype(pMashTag)))
	{
		_MSG_BOX("CSmall_Object Created Failed");
...
CGameObject * CSmall_Object::Clone(const _vec3 * pPos)
{
	CGameObject*	pInstance = new CSmall_Object(*this);

	if (FAILED(pInstance->Ready_GameObject_Clone(pPos)))
	{
		_MSG_BOX("CSmall_Object Clone Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CSmall_Object::Clone(const _matrix & matWorld)
{
	CGameObject*	pInstance = new CSmall_Object(*this);
