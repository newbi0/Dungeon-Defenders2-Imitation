...
HRESULT CSword::Ready_GameObject_Prototype(void)
{
...
HRESULT CSword::Ready_GameObject_Clone(const _vec3* pPos)
{
...
HRESULT CSword::Ready_GameObject_Clone(const _matrix& matWorld)
{
...
void CSword::Update_Collider(void)
{
...
void CSword::Collision_Att(const _tchar* pTargetTag)
{
	if (true == m_isAtt)
	{
...
void CSword::Render_GameObject(void)
{
	...
	{
		_MSG_BOX("CSword SetUp_ConstantTable Failed");
		return;
	}
...
CSword * CSword::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	...
	{
		_MSG_BOX("CSword Created Failed");
...
CGameObject * CSword::Clone(const _vec3 * pPos)
{
	CGameObject*	pInstance = new CSword(*this);

	if (FAILED(pInstance->Ready_GameObject_Clone(pPos)))
	{
		_MSG_BOX("CSword Clone Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CSword::Clone(const _matrix & matWorld)
{
	CGameObject*	pInstance = new CSword(*this);
