...
HRESULT CLight::Ready_GameObject_Prototype(const _tchar* pMashTag)
{
...
HRESULT CLight::Ready_GameObject_Clone(const _vec3* pPos)
{
...
HRESULT CLight::Ready_GameObject_Clone(const _matrix& matWorld)
{
	m_pRendererCom = (Engine::CRenderer*)Get_Component(L"Com_Renderer");
	m_pRendererCom->AddRef();
...
HRESULT CLight::Ready_Component(const _tchar* pMashTag)
{
...
HRESULT CLight::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
...
_int CLight::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (nullptr == m_pRendererCom)
		return -1;
...
void CLight::Render_GameObject(void)
{
	if (nullptr == m_pTransformCom
	...
	{
		_MSG_BOX("CLight SetUp_ConstantTable Failed");
		return;
	}
...
CLight * CLight::Create(LPDIRECT3DDEVICE9 pGraphic_Device, const _tchar* pMashTag)
{
...
CGameObject * CLight::Clone(const _vec3 * pPos)
{
	CGameObject*	pInstance = new CLight(*this);

	if (FAILED(pInstance->Ready_GameObject_Clone(pPos)))
	{
		_MSG_BOX("CLight Clone Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CLight::Clone(const _matrix & matWorld)
{
	CGameObject*	pInstance = new CLight(*this);
