...
CSign_Win::CSign_Win(LPDIRECT3DDEVICE9 pGraphic_Device)
	: CGameObject(pGraphic_Device)
{
}

CSign_Win::CSign_Win(const CSign_Win & rhs)
	: CGameObject(rhs)
{
}

HRESULT CSign_Win::Ready_GameObject_Prototype(void)
{
...
HRESULT CSign_Win::Ready_GameObject_Clone(const _vec3* pPos)
{
	...
	m_pBufferCom->Resize_Buffer(_vec2(-0.6f, -0.5f), _vec2(0.6f, 0.5f));

	return NOERROR;
}

HRESULT CSign_Win::Ready_GameObject_Clone(const _matrix& matWorld)
{
...
HRESULT CSign_Win::Ready_Component(void)
{
	...
	// For.Com_Texture
	m_pTextureCom = (Engine::CTexture*)m_pComponent_Manager->Clone_Component(SCENE_STAGE, L"Com_Texture_Win");
...
HRESULT CSign_Win::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
...
_int CSign_Win::Update_GameObject(const _float & fTimeDelta)
{
	return _RUN;
}

_int CSign_Win::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (nullptr == m_pRendererCom)
		return -1;
...
void CSign_Win::Render_GameObject(void)
{
	if (nullptr == m_pTransformCom
	...
	{
		_MSG_BOX("CSign_Win SetUp_ConstantTable Failed");
		return;
	}
...
CSign_Win * CSign_Win::Create(LPDIRECT3DDEVICE9 pGraphic_Device)
{
	CSign_Win* pInstance = new CSign_Win(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype()))
	{
		_MSG_BOX("CSign_Win Created Failed");
...
CGameObject * CSign_Win::Clone(const _vec3 * pPos)
{
	CGameObject*	pInstance = new CSign_Win(*this);

	if (FAILED(pInstance->Ready_GameObject_Clone(pPos)))
	{
		_MSG_BOX("CSign_Win Clone Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CSign_Win::Clone(const _matrix & matWorld)
{
	CGameObject*	pInstance = new CSign_Win(*this);
