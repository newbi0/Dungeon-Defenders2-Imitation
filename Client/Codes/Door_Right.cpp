...
HRESULT CDoor_Right::Ready_GameObject_Prototype(const _tchar* pMashTag)
{
...
HRESULT CDoor_Right::Ready_GameObject_Clone(const _vec3* pPos)
{
...
HRESULT CDoor_Right::Ready_GameObject_Clone(const _matrix& matWorld)
{
...
HRESULT CDoor_Right::Ready_Component(const _tchar* pMashTag)
{
...
HRESULT CDoor_Right::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
...
HRESULT CDoor_Right::SetUp_LightAndMaterialInfo_OnShader(LPD3DXEFFECT pEffect)
{
	//LightInfo
...
_int CDoor_Right::Update_GameObject(const _float & fTimeDelta)
{
	...
	{
		m_isOpen = false;
		m_pTransformCom->RotationY(-3.f);
	}

	return _int();
}

_int CDoor_Right::LastUpdate_GameObject(const _float & fTimeDelta)
{
	if (nullptr == m_pRendererCom)
		return -1;
...
void CDoor_Right::Render_GameObject(void)
{
	if (nullptr == m_pTransformCom
	...
	{
		_MSG_BOX("CDoor_Right SetUp_ConstantTable Failed");
		return;
	}
...
CDoor_Right * CDoor_Right::Create(LPDIRECT3DDEVICE9 pGraphic_Device, const _tchar* pMashTag)
{
	CDoor_Right* pInstance = new CDoor_Right(pGraphic_Device);

	if (FAILED(pInstance->Ready_GameObject_Prototype(pMashTag)))
	{
		_MSG_BOX("CDoor_Right Created Failed");
...
CGameObject * CDoor_Right::Clone(const _vec3 * pPos)
{
	CGameObject*	pInstance = new CDoor_Right(*this);

	if (FAILED(pInstance->Ready_GameObject_Clone(pPos)))
	{
		_MSG_BOX("CDoor_Right Clone Failed");
		Safe_Release(pInstance);
	}
	return pInstance;
}

CGameObject * CDoor_Right::Clone(const _matrix & matWorld)
{
	CGameObject*	pInstance = new CDoor_Right(*this);
