...
HRESULT CStatic_Object::Ready_GameObject_Prototype(const _tchar* pMashTag)
{
...
HRESULT CStatic_Object::Ready_GameObject_Clone(const _vec3* pPos)
{
...
HRESULT CStatic_Object::Ready_GameObject_Clone(const _matrix& matWorld)
{
...
HRESULT CStatic_Object::Ready_Component(const _tchar* pMashTag)
{
...
HRESULT CStatic_Object::SetUp_ConstantTable(LPD3DXEFFECT pEffect)
{
	...
	//_vec4	vCamPos = _vec4((dynamic_cast<CCamera*>(CObject_Manager::Get_Instance()->Get_GameObject(SCENE_STAGE, L"Layer_Camera"))->Get_Camera_Desc())->vEye, 1.f);
	//if (FAILED(pEffect->SetVector("g_vCameraPosition", &vCamPos)))
	//	return E_FAIL;
...
HRESULT CStatic_Object::SetUp_LightAndMaterialInfo_OnShader(LPD3DXEFFECT pEffect)
{
	//LightInfo
...
void CStatic_Object::Render_GameObject(void)
{
	if (nullptr == m_pTransformCom
...
CStatic_Object * CStatic_Object::Create(LPDIRECT3DDEVICE9 pGraphic_Device, const _tchar* pMashTag)
{
...
CGameObject * CStatic_Object::Clone(const _vec3 * pPos)
{
	CGameObject*	pInstance = new CStatic_Object(*this);
...
CGameObject * CStatic_Object::Clone(const _matrix & matWorld)
{
	CGameObject*	pInstance = new CStatic_Object(*this);
